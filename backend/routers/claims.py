...
async def scrub_claim_endpoint(claim_data: ClaimFormData, current_user = Depends(get_current_user)):
    ...
    # 1. Resolve payer (registered insurance company in `insurers`)
    ...
    # 2. Resolve clinic (provider_org). Either an explicit org the doctor is
    #    linked to via doctor_org_links, or fall back to their primary affiliation.
    ...
    if claim_data.clinic_id and is_valid_uuid(claim_data.clinic_id):
        try:
            ...
            if link.data:
                ...
                logger.warning(
                    f"User {user_id} tried to submit for org {claim_data.clinic_id} without an active link."
                )
        except Exception as e:
    ...
    claim_payload = {
        ...
        "status": "pending",
        "user_id": user_id,                 # submitting doctor / provider admin
        "clinic_id": resolved_clinic_id,    # billing organization
        "payer_id": resolved_payer_id,      # insurer (NULL if unregistered)
